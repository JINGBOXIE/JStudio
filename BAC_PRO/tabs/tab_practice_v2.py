# ---------------------------------------------------------
# tab_practice_v2.py - 完全修复AUTORUN下注，但是不设限，直到账户余额不足
# ---------------------------------------------------------
...
def render_practice_tab(lang):
    ...
    def run_auto_engine():
        """
        V3 引擎：集成了自动下注、自动续靴、自动驱动
        """
        # A. 自动续靴逻辑：如果靴子结束了，立即重置
        if st.session_state.get('end_shoe', False):
            if st.session_state.auto_run_active:
                reset_logic()
                st.toast("🔄 自动开启新牌靴 (Auto-New Shoe)")
                time.sleep(0.5)
                # 重置后不 return，直接向下执行新靴的第一手
        ...
        if adapter and clean_seq:
            ...
            if decision and decision.get('action'):
                ...
                if is_con: target_side = clean_seq[-1]
                elif is_cut: target_side = 'P' if clean_seq[-1] == 'B' else 'B'
