...
公开方法
  ...
  ._terminate()                                     内部重置，外部不应调用但保留（SettlementCenter 可能引用）
...
_locked_streak_side : _terminate() 后锁住该方向，阻止同方向STREAK重复开仓
...
class StrategyEngine:
    ...
    def _terminate(self):
        """
        将状态机归零，同时锁定本次S1所属的STREAK方向。

        锁定逻辑：
          - _active_streak_side 在 start() 时由Orchestrator传入并保存
          - _terminate() 用它来锁定 _locked_streak_side
          - 这确保锁住的是"本次S1执行时的STREAK方向"，而不是终止帧的cur_side
          - 例如：B STREAK下注后P出现（STREAK反转），终止帧cur_side="P"，
            但应锁住"B"，防止的是同一B STREAK重复开仓，而非新P STREAK
        """
        streak_to_lock           = self._active_streak_side  # 保存再清除
        self.state               = "SLEEP"
        self.step_index          = 0
        self.strategy_key        = None
        self.active_side         = None
        self._streak_executed    = False
        self._active_streak_side = None
        # _locked_streak_side 设为本次S1所属方向，由 unlock_streak() 在反转时解除
