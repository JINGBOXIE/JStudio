...
def render_imarket_sidebar(lang):
    # 1. 统一语言标识
    report_lang = "English" if lang == "EN" else "中文"
    
    # 获取当前股票代码，统一定义变量名以防止后续报错
    ticker = st.session_state.get("ticker_input_val", "AAPL").upper()
    ticker_input = ticker
# 2. 核心数据资产
    WATCHLIST_DATA = {
    ...
    with col_input:
        ticker_input_val = st.text_input(
                "Ticker (AAPL | TSLA)" if lang == "EN" else "股票代码 (AAPL | TSLA)",
                value=st.session_state.get("ticker_input_val", "AAPL"),
    ...
    with col_pop:
        ...
        with st.popover(">>", help="Quick Watchlist"):
            st.markdown(f"### {'Select Ticker' if lang == 'EN' else '自选股票池'}")
    ...
    if st.button(btn_label, key="btn_integrated_analysis", help=btn_help, use_container_width=True, type="primary"):
        with st.spinner("Analyzing..." if report_lang == "English" else "分析中..."):
            index_data = {"Oil": "$103", "Rates": "3.5%-3.75%", "VIX": "Tracking", "Region": "Middle East"}
...
def render_imarket_tab(lang):
    """
    这个函数是主程序的入口。
    必须把 with st.sidebar 放在这个函数内部，它才能被渲染出来。
    """

    # --- 修复补丁：定义 report_lang 以兼容原有逻辑 ---
    if 'report_lang' not in globals():
        report_lang = "English" if st.session_state.get('lang') == "EN" else "中文"
    # ----------------------------------------------
    # --- 核心修复：确保主页面函数内同时拥有这两个变量名 ---
    # 从 session 中提取值
    current_symbol = st.session_state.get("ticker_input_val", "AAPL").upper()
    
    # 定义 ticker_input (用于第 446 行的大标题)
    ticker_input = current_symbol
    
    # 定义 ticker (用于第 457 行的数据抓取和后续逻辑)
    ticker = current_symbol
    lookback = st.session_state.get("lookback_val", 30)
