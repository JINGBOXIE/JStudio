...
# 根据截图，两个文件夹都在 GitHub 目录下，所以直接获取当前目录的父目录
current_dir = os.path.dirname(os.path.abspath(__file__))
parent_dir = os.path.dirname(current_dir)
finance_system_path = os.path.join(parent_dir, "unified_finance_system")

if finance_system_path not in sys.path:
    sys.path.append(finance_system_path)
...
try:
    ...
    # 初始化财务系统 (模式跟随 .env)
    fs = FinanceSystem(mode=Config.RUN_MODE)
...
# --- [新增：初始化 Session State] ---
# 必须在读取 auth_user 之前运行这段代码
...
# --- 2. 语言选择器 (必须在身份验证之前定义，以解决 NameError 和双语问题) ---
with st.sidebar:
    # 只有一个 key='lang_selector'，确保唯一
    report_lang = st.selectbox(
        "🌐 Language / 语言", 
        ["English", "中文"], 
        index=0 if st.session_state.get('lang_selector', 'English') == "English" else 1,
        key='lang_selector'
    )
    # 定义这个变量以兼容后续代码
    ...
    if st.session_state.auth_user is None:
        st.subheader("🔑 身份验证" if report_lang == "中文" else "🔑 Authentication")
        
        # value="" 确保输入框初始为空白，取消 1 作为默认值
        u_id_raw = st.text_input(
            "UID (J / D / A)", 
            value="", 
            placeholder="Type J",
            key="login_input"
        ...
        btn_text = "进入系统" if report_lang == "中文" else "Enter System"
        if st.button(btn_text, use_container_width=True, type="primary"):
            if u_id_raw in ['J', 'D', 'A']:
                # 查询云端数据
                acc_check = fs.get_balance(u_id_raw)
                if acc_check:
                    st.session_state.auth_user = u_id_raw
                    st.rerun()
                else:
                    err_msg = f"❌ ID '{u_id_raw}' 未在云端注册" if report_lang == "中文" else f"❌ ID '{u_id_raw}' not registered"
                    st.error(err_msg)
            elif u_id_raw == "":
                st.warning("⚠️ 请输入 UID" if report_lang == "中文" else "⚠️ Please enter UID")
            else:
                st.error("🚫 仅限 J, D, A" if report_lang == "中文" else "🚫 J, D, A Only")
    ...
    acc = fs.get_balance(uid)
    
    if acc:
        col_u, col_l = st.columns([2, 1])
        with col_u:
            st.write(f"👤 **UID: {uid}**")
            st.caption(f"💰 Balance: ${acc.get('balance', 0.0):,.2f}") 
        with col_l:
            exit_text = "退出" if report_lang == "中文" else "Exit"
            if st.button(exit_text, use_container_width=True):
                st.session_state.auth_user = None
                st.rerun()
    ...
    if st.sidebar.button(btn_label, key="btn_integrated_analysis", help=btn_help, use_container_width=True, type="primary"):
        # 尝试执行扣费 (例如每次分析扣除 $10.0)
        payment = fs.place_bet(uid, "iMarket", 10.0, {"action": "AI_Analysis", "ticker": ticker})
        ...
        if payment["status"] == "success":
            ...
            # 如果余额不足，直接在侧边栏提示
            st.sidebar.error(f"❌ 支付失败: {payment['message']}")
