...
def run_v3_specialized_report(ticker, segment, data_payload, lang="中文"):
    ...
    # 2. 自动化 API 配置 (新版 Client，复用 session_state)
    try:
    ...
    # 5. 构造带时间锁的 System Prompt + 高强度分析 Prompt
    ...
    # 6. 执行请求 (新版 SDK，含模型降级保护)
    # 优先使用预览版，云端未就绪时自动降级到稳定版
    model_priority = [
        "gemini-2.5-flash-preview",   # V3.3 首选
        "gemini-2.0-flash",            # 降级 · 稳定版
        "gemini-1.5-flash",            # 降级 · 兼容版
    ]
    ...
    return f"❌ AI Engine 报告生成失败 (所有模型均不可用): {last_error}"
